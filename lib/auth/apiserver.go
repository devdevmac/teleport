...
func NewAPIServer(config *APIConfig) http.Handler {
	...
	srv.GET("/:version/ca", srv.withAuth(srv.getLocalCA))
...
// getLocalCA returns the CAs for the local cluster without signing keys.
func (s *APIServer) getLocalCA(auth ClientI, w http.ResponseWriter, r *http.Request, p httprouter.Params, version string) (interface{}, error) {
	localCA, err := auth.GetLocalCA()
	if err != nil {
		return nil, trace.Wrap(err)
	}
