...
func insecureRegisterClient(params RegisterParams) (*Client, error) {
	...
	cert, foundCA, err := readCA(params)
	if err != nil {
		return nil, trace.Wrap(err)
	}
	if foundCA {
...
func readCA(params RegisterParams) (*x509.Certificate, bool, error) {
	...
	if err != nil {
		return nil, false, trace.Wrap(err, "failed to parse certificate at %v", params.CAPath)
	}
